...
<span class="productName">بحث عن طريق:</span><br>
	...
	<form action="../Controller/ProductController.php" method="post">
		...
		<input class="numbut"type="submit" name="search_by_name" value="بحث" style="width:100px;">
	</form>
	
	<form action="../Controller/ProductController.php" method="post">
		<span  class="productName">نوع الصنف</span>
		<select style ="margin-right:43px;" class="option" id="category" name="category">
			<?php
		...
		<input style ="margin-right:66px;width:100px;"class="numbut"type="submit" name="search_by_category" value="بحث">
	</form>
	
	<form action="../Controller/ProductController.php" method="post">
		<span  class="productName">ب:</span>
		<select style ="margin-right:120px;" class="option" name="EAV_ID">
			<?php
				require_once("../Model/ProductTypeAttributeModel.php");
				$ProductTypeAttribute=new ProductTypeAttributeModel("");
				$result=$ProductTypeAttribute->Select_All_Attributes();
				for($i = 0; $i < count($result); $i++){
					echo "<option id='EAVoption' value=".$result[$i]->get_Attribute_ID().">".$result[$i]->get_Attribute_Name()."</option>";
				}
			?>
		</select>
		<input style ="margin-right:66px; width:100px;"class="numbut"type="submit" name="search_by_EAV" value="بحث">
	</form>
	...
	<?php
		class ProductView
		{
			public function displayEAV($attData,$eavData,$prodData)
			{
				if($prodData!=null)
				{
					echo "<tr><th>الصنف</th> <th>نوع الصنف</th> <th>القيمة</th> <th>نوع القيمة</th> </tr>";
					for($i=0;$i<sizeof($prodData);$i++)
					{
						echo"<tr>".
							"<td>".$prodData[$i]->get_product_name()."</td>".
							"<td>".$prodData[$i]->get_product_CategoryObj()->get_name()."</td>".
							"<td>".$eavData[$i]->get_Value()."</td>".
							"<td>".$attData->get_Attribute_Name()."</td>";
					}
					echo "</tr></table>";
				}
			}
			public function displayProduct($result)
			{
				if($result!=null)
				{
					echo "<tr><th>الصنف</th> <th>نوع الصنف</th> </tr>";
...
function search(cat_ID,req)
{
	xmlHttp = new XMLHttpRequest();
		...
		xmlHttp.open('GET', '../Controller/ProductController.php?cat_ID='+cat_ID+'&req='+req, true);
		xmlHttp.send();
}
function displayTable(prodArr)
{
	var prod=prodArr[0];
	var cat=prodArr[1];
...
function action(actionType,prodID)
{
	console.log(actionType+" "+prodID);
	xmlHttp = new XMLHttpRequest();
        xmlHttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                console.log("request finished and response is ready");
		...
		if(actionType=="delete")
			xmlHttp.open('GET', 'DeleteProduct.php?&prodID='+prodID, true);
		else if(actionType=="edit")
			xmlHttp.open('GET', '../Controller/ProductController.php?&prodID='+prodID, true);
